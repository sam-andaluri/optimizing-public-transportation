...
class KafkaConsumer:
    ...
    def __init__(
        ...
        #
        group_name = re.sub(r"[^a-zA-Z0-9._-]+", "_", topic_name_pattern)
        self.broker_properties = {
            "bootstrap.servers": os.getenv(
                "KAFKA_BOOTSTRAP_SERVERS", "localhost:9092"
            ),
            "group.id": f"cta-{group_name}",
            "auto.offset.reset": "earliest" if offset_earliest else "latest",
        }

        if is_avro is True:
            self.broker_properties["schema.registry.url"] = os.getenv(
    ...
    def _consume(self):
        ...
        try:
            ...
            logger.exception("message consumption failed")
            return 0
